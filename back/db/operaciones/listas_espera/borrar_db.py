...
def borrar_listas_espera_individuales_clase(id_clase: int, cursor):
    """Permite eliminar una o mas filas para la tabla Lista_Espera_Individual para una clase"""
    query = f"""
        DELETE FROM Lista_Espera_Individual
        WHERE inst_clase_id IN (SELECT ic.id
               FROM Instancia_Clase ic INNER JOIN Clase c ON (ic.clase_id = c.id)
               WHERE c.id = {id_clase});
    """
    return ejecutar_query(query, cursor)

def borrar_listas_espera_abonados_clase(id_clase: int, cursor):
    """Permite eliminar una o mas filas para la tabla Lista_Espera_Abonados"""
    query = f"""
